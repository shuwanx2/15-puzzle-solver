...
public:
    ...
    */
    ...
    bool isValidState(const std::array<uint8_t, 16>& state) const;
    /**
    ...
    * 12 13 14 15
    */
    PuzzleState(const std::array<uint8_t, 16> state);//state checking
...
private:
    std::array<uint8_t, 16> state_; // private member variable (add)
    int emptyIndex;
};
